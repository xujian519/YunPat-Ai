...
/// 统一的项目列表面板（替代案件列表 / 案件工作区 / 知识库）
struct ProjectListSidebar: View {
    @ObservedObject var tabManager: TabManager
    @State private var filterCategory: ProjectFilter = .all
    ...
    private var filterPicker: some View {
        Picker("筛选分类", selection: $filterCategory) {
            ForEach(ProjectFilter.allCases, id: \.self) { flag in
    ...
    private var tabList: some View {
        Group {
            if filteredTabs.isEmpty {
                EmptyStateView(
                    icon: "tray",
                    title: filterCategory == .archived ? "暂无归档" : "无项目",
    ...
    private var footer: some View {
        VStack(spacing: 0) {
            Divider()
            HStack {
                Text("共 \(tabManager.tabs.count) 个项目")
                    .font(FontStyle.caption)
                    .foregroundStyle(.secondary)
                    .accessibilityLabel("共 \(tabManager.tabs.count) 个项目")
                Spacer()
