...
public actor MemoryReadPath {
    ...
    public func assemble(for query: String, caseId: String) async -> MemoryBlock? {
        let store: MemoryStore = MemoryStore()
        ...
        // 加载 LTM 补充
        let ltm: LongTermMemory = await store.loadLongTermMemory()
        let ltmRelevant: [MemoryItem] = ltm.items.filter { relevance(query, $0.content) >= relevanceThreshold }
    ...
    private func relevance(_ query: String, _ target: String) -> Double {
        ...
        let queryWords = Set(
            query.lowercased().components(separatedBy: CharacterSet.alphanumerics.inverted).filter { $0.count > 1 })
        let targetWords = Set(
            target.lowercased().components(separatedBy: CharacterSet.alphanumerics.inverted).filter { $0.count > 1 })
...
public struct MemoryBlock: Sendable {
    ...
    public init(
        technicalField: String? = nil, inventionPoints: [String] = [], keyReferences: [String] = [],
        openIssues: [String] = [], longTermRelevant: [String] = []
    ) {
        self.technicalField = technicalField
        self.inventionPoints = inventionPoints
        self.keyReferences = keyReferences
        self.openIssues = openIssues
        self.longTermRelevant = longTermRelevant
    }

    /// 渲染为 LLM 可注入的文本
    public var rendered: String {
        var lines: [String] = []
        if let techField = technicalField { lines.append("技术领域: \(techField)") }
