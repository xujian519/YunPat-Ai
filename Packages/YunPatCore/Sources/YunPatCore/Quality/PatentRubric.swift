...
public struct PatentRubric: Sendable {
    ...
    public static let drafting: PatentRubric = PatentRubric(criteria: [
    ...
    public var verdict: RubricVerdict {
        let below: [RubricCriterion] = criteria.filter { $0.score < minPerCriterion }
    ...
    public func report() -> String {
        var lines: [String] = ["## 质量评分报告", "", "| 维度 | 得分 | 满分 | 状态 |", "|------|------|------|------|"]
        for criterion in criteria {
            let status: String = criterion.score >= 4 ? "✅" : criterion.score >= minPerCriterion ? "⚠️" : "❌"
            lines.append("| \(criterion.name) | \(criterion.score) | \(criterion.maxScore) | \(status) |")
        }
        lines.append("")
        ...
        case .pass:
            lines.append("**✅ 通过**")
        case .conditionalPass(let items):
            lines.append("**⚠️ 有条件通过** (\(items.joined(separator: "、"))")
        case .fail(let items, let score):
            lines.append("**❌ 不通过** (总分\(score), 薄弱: \(items.joined(separator: "、"))")
        }
