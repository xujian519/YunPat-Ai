...
final class MemoryEngineTests: XCTestCase {
    ...
    func testWorkingMemory_setAndRetrieveGoal() async {
        ...
        let goal = await engine.currentGoal()
    ...
    func testWorkingMemory_hypothesesAndScratchpad() async {
        ...
        let hyps = await engine.activeHypotheses()
        let notes = await engine.scratchpad()
    ...
    func testWorkingMemory_intermediateResults() async {
        ...
        let count = await engine.result(forKey: "claimCount")
    ...
    func testWorkingMemory_resetClearsEverything() async {
        ...
        await engine.setResult("v", forKey: "k")
        await engine.resetWorkingMemory()
        let goal = await engine.currentGoal()
        let hyps = await engine.activeHypotheses()
        let notes = await engine.scratchpad()
        let result = await engine.result(forKey: "k")
        XCTAssertEqual(goal, "")
    ...
    func testSessionFacts_addAndFilter() async {
        ...
        let all = await engine.pendingSessionFacts()
        XCTAssertEqual(all.count, 3)

        let tech = await engine.sessionFacts(ofCategory: .technicalFeature)
    ...
    func testCaseContext_saveAndLoad() async throws {
        ...
        let loaded = await engine.loadCaseContext("test-case-1")
    ...
    func testCaseContext_remove() async throws {
        let engine = MemoryEngine()
        let ctx = CaseContext(caseId: "to-remove")
        try await engine.saveCaseContext(ctx)
        await engine.removeCaseContext("to-remove")
        let loaded = await engine.loadCaseContext("to-remove")
        XCTAssertNil(loaded)
    }
    ...
    func testLongTermMemory_addPrecedent() async throws {
        ...
        let ltm = await engine.loadLongTermMemory()
    ...
    func testGlobalMemory_saveAndLoad() async throws {
        ...
        try await engine.saveGlobalMemory(gm)
        let loaded = await engine.loadGlobalMemory()
    ...
    func testConsolidate_clearsSessionFactsAndSavesCaseContext() async throws {
        ...
        let ctx = try await engine.consolidate()
        ...
        let remaining = await engine.pendingSessionFacts()
    ...
    func testConsolidateDeep_promotesStrategiesToLongTermMemory() async throws {
        ...
        let result = try await engine.consolidateDeep()
        let ctx = result.0
        let ltm = result.1
    ...
    func testFiveLayerIntegration() async throws {
        ...
        let gm = await engine.loadGlobalMemory()
        XCTAssertEqual(gm.writingStyle, "权威学术风格")
    }
}
