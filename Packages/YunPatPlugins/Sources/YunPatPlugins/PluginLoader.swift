...
public actor PluginLoader {
    ...
    public func loadManifest(from path: URL) async throws -> PluginManifest {
        ...
        guard let manifestURL = bundle.url(forResource: "manifest", withExtension: "json"),
              ...
              let manifest = try? JSONDecoder().decode(PluginManifest.self, from: data) else { throw PluginError.manifestNotFound }
        loadedBundles[manifest.id] = bundle
        return manifest
    }

    public func unload(_ pluginID: String) { loadedBundles[pluginID]?.unload(); loadedBundles[pluginID] = nil }
}
